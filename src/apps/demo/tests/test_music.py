...
class Test(TestCase):
    def setUp(self):
        ...
        instances = []
        for i in range(10):
            instances.append(Music(**self.seeder.seed(Music)))
        Music.objects.bulk_create(instances)

    def test_query(self):
        self.assertGreaterEqual(Music.objects.count(), 10)
    ...
    def test_api_retrieve(self):
        ...
        resp = self.client.get(reverse_lazy('api:demo/music/retrieve', kwargs={'item_id': item['data']['id']}))
        self.assertEqual(resp.status_code, 200)

    def test_api_list(self):
        resp = self.client.get(reverse_lazy('api:demo/music/list'))
        self.assertEqual(resp.status_code, 200)
    ...
    def test_api_destroy(self):
        ...
        resp = self.client.delete(
            reverse_lazy('api:demo/music/destroy', kwargs={'item_id': item['data']['id']}),
            content_type='application/json'
        )
        self.assertEqual(resp.status_code, 200)
        self.assertIsNotNone(resp.json())
